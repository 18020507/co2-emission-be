...
@router.get(Route.V1.GET_FACILITY_MASTER_DATA, dependencies=[Depends(login_required)])
async def get_company_facility_master_data(company_id: int):
    logging.info("===>>> company_facility_master_data_controller.py <<<===")
    logging.info("===>>> function get_company_facility_master_data <<<===")
    try:
        response = await company_facility_master_data_service.get_company_facility_master_data(company_id)
        return response
